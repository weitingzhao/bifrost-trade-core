...
from bifrost_core.monitor.reader.gate_safety import _row_to_gates
...
def test_payload_to_row_flattens_nested_gates():
    payload = {
        "name": "Default",
        "version": 2,
        "dim_direction": "long",
    ...
    assert row["epsilon_band"] == 10
    assert row["min_hedge_shares"] == 10
    assert row["max_daily_hedge_count"] == 50
    assert row["paper_trade"] is True
    assert dates == ["2026-04-15", "2026-07-20"]


def test_row_to_gates_preserves_config_shape():
    row = {
        "min_dte": 21,
        "max_dte": 35,
        "atm_band_pct": 0.03,
